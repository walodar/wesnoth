...
if wesnoth.kernel_type() == "Game Lua Kernel" then
	...
	function wesnoth.hooks.register(name, arg1, arg2)
		...
		local hl = hooks[name]
		table.insert(hl, {h = handler, p = priority})
		-- sort it.
		for i = #hl - 1, 1, -1 do
	...
	-- Unregister a handler. Requires a direct reference to the handler function.
	function wesnoth.hooks.unregister(name, fcn)
		if hooks[name] == nil then return end
		for i = #hooks[name], 1, -1 do
			if hooks[name][i].h == fcn then
	...
	local hooks_mt = {
		...
		-- You can assign a handler with default priority (which is 0, the highest)
		...
		__index = function(_, k)
			local matching = {}
			if hooks[k] == nil then return matching end
			for i = 1, #hooks[k] do
	...
	wesnoth.hooks = setmetatable(wesnoth.hooks, hooks_mt)
end
